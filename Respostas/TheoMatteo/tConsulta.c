...
Vector* ObtemConsultasCPF (char* cpf, FILE* file) {
    ...
    unsigned int buffer_size = TAM_MAX_CPF + TAM_MAX_DATA + TAM_MAX_TIPO_PELE + TAM_CRM + sizeof(int) * 4;
    ...
    while(true) {
        ...
        if (fread(cpfPaciente, sizeof(char), TAM_MAX_CPF, file) != TAM_MAX_CPF) break;

        if (!strcmp(cpf, cpfPaciente)) {

            tConsulta* consulta = AlocaConsulta();

            strcpy(consulta->cpfPaciente, cpfPaciente);
            fread(consulta->cpfMedico, sizeof(char), TAM_MAX_CPF, file);
            fread(consulta->CRM, sizeof(char), TAM_CRM, file);
            fread(consulta->data, sizeof(char), TAM_MAX_DATA, file);
            fread(consulta->tipoPele, sizeof(char), TAM_MAX_TIPO_PELE, file);
            fread(&consulta->possui_diabetes, sizeof(int), 1, file);
            fread(&consulta->eh_fumante, sizeof(int), 1, file);
            fread(&consulta->possui_alergia, sizeof(int), 1, file);
            fread(&consulta->possui_HistCancer, sizeof(int), 1, file);

            VectorPushBack(v, consulta);
        }
        else {
...
int EhFumante(tConsulta* c) {
    return c->eh_fumante;
}


int PossuiDiabetesConsulta(tConsulta* c) {
    return c->possui_diabetes;
}


int PossuiAlergiaConsulta(tConsulta* c) {
    return c->possui_alergia;
}


int PossuiHistoricoCancer(tConsulta* c) {
    return c->possui_HistCancer;
}

char* TipoPeleConsulta(tConsulta* c) {
    return c->tipoPele;
}
...
void DesalocaConsulta (void* dado) {
    ...
    if (c->lesoes) VectorDestroy(c->lesoes, DesalocaLesao);
    free(c);
}