...
tRelatorio* GeraRelatorioCompleto(tDatabase* database, tConsulta** consultas, int qtdConsultas) {
    ...
    PreencheNumeroAtendRelatorio(consultas, qtdConsultas, r);
...
void PreencheNumeroAtendRelatorio (tConsulta** consultas, int qtd, tRelatorio* r) {
    ...
    for (int i = 0; i < qtd; i++) {
        ...
        for (int j = 0; j < i; j++) {
            if (strcmp(ObtemCPFPacienteConsulta(consultas[i]), ObtemCPFPacienteConsulta(consultas[j])) == 0) {
                flag = true;
                break;
            }
        }
    ...
    r->totalPacientesAtendidos = count;
}
