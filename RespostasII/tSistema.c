...
struct tSistema {
    ...
    // Consultas realizadas no sistema
    tConsulta** consultas;
    int qtdConsultas;
};
...
void IniciaSistema (tSistema* s) {
    ...
    // Salva consultas realizadas durante o programa
    SalvaConsultasArquivoBinario(s->consultas, s->qtdConsultas, ObtemArquivoConsultas(database));
}
...
void ExecutaRelatorio (tSistema* s) {
    ...
    tRelatorio* relatorio = GeraRelatorioCompleto(s->database, s->consultas, s->qtdConsultas);
