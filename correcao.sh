                        ...
                        for src_file_dir in "${src_files_names[@]}"; do
                            ...
                            for DIR_CASE in "$DIR_CASES"/*; do
                                ...
                                if [ "$IGNORE_VALGRIND" = "false" ]; then
                                    run_output=$(timeout 2 valgrind $valgrind_args $binary $directory_path < $input_file > $output 2>&1)
                                else
                                    run_output=$(timeout 2 $binary $directory_path < $input_file > $output 2>&1)
                                fi
                    ...
                    if [ "$IGNORE_RESULTS" = "false" ] || [ "$IGNORE_VALGRIND" = "false" ]; then
                        ...
                        for src_file_dir in "${src_files_names[@]}"; do
                            ...
                            for DIR_CASE in "$DIR_CASES"/*; do
                                ...
                                if [ "$IGNORE_VALGRIND" = "false" ]; then
                                    output=$(timeout 2 valgrind $valgrind_args $binary $directory_path < $input_file > $output 2>&1)
                                else
                                    output=$(timeout 2 $binary $directory_path < $input_file > $output 2>&1)
                                fi
